...
class TestUserAuth:
    def test_auth_user(self):
        ...
        assert "auth_sid" in response1.cookies, "There is no cookie in the response1"
    ...
    def test_negative_auth_check(self, condition):
        ...
        if condition == "no_cookie":
            response2 = requests.get(url2, headers={"x-csrf-token": token})
        else:
            response2 = requests.get(url2, cookies={"auth_sid": auth_sid})
